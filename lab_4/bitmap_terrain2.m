...
output_dir = 'rotated_frames';
if ~exist(output_dir, 'dir')
    mkdir(output_dir);
end

output_gif = fullfile(output_dir, 'terrain_rotation_gif.gif');

for angle = 0:10:350
    surf(X,Y,Z);
    
    view(angle, 30);  % 30 Y, spin Z
    
    axis tight;
    axis off;
    
    filename = fullfile(output_dir, sprintf('terrain_rotation_%03d.jpg', angle));
    saveas(gcf, filename);  % Zapisuje bieżący widok figury jako plik JPG
    

    frame = getframe(gcf); % Zapisz bieżący obraz jako klatkę
