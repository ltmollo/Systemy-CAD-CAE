...
%bitmap init
bitmap = im2gray(imread("obraz.jpg"));
bitmap_size = size(bitmap);

knot_vector_x=0:bitmap_size(1)-1;
knot_vector_x(1:3) = 0;
for i = 4:bitmap_size(1)
    knot_vector_x(i) = i-3;
end
knot_vector_x(end-2 : end) = bitmap_size(1)-5;
knot_vectory = knot_vector_x; 
...
function save_plot(u, iter, bx, by)
  ...
  output_dir = 'lab_5/results';
  if ~exist(output_dir, 'dir')
      mkdir(output_dir);
  end
  output_gif = fullfile(output_dir, 'result.gif');
  ...
  saveas(h, sprintf('lab_5/results/out_%d.png', iter));

  % save to gif
  frame = getframe(h); % Zapisz bieżący obraz jako klatkę
  im = frame2im(frame);  % Konwersja klatki do RGB (macierz pikseli)
  [imind, cm] = rgb2ind(im, 256);
  % rgb2ind - konwertuje obraz na indeksowany, redukując liczbę kolorów
  %iming - macierz indeksowana, czyli obraz, w którym każdy piksel jest zapisany jako indeks koloru.
  %cm - mapa kolorów tabela kolorów, która przypisuje indeksy do konkretnych kolorów.
...
function y=init_state_ball(x)
  ...
  alpha=1; %changed from 20, gotta test if it works
  lambda=12;
